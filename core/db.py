...
class DB:
        ...
        with self._connect() as conn:
            conn.executescript("""
                CREATE TABLE IF NOT EXISTS bets (
                    ...
                    outcome       TEXT DEFAULT NULL,  -- "win" | "loss" | "pending"
            ...
            """)
        log.info(f"[db] SQLite 초기화 완료: {self._path}")
    ...
    def insert_bet(
        ...
        with self._connect() as conn:
            cur = conn.execute(
                """
                INSERT INTO bets
                  (game_id, event_title, question, token_id, favorite_team,
                   pinnacle_odds, pinnacle_prob, poly_price, gap_size,
                   bet_usdc, order_id, commence_time, bet_at, outcome)
                VALUES
                  (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
                """,
                (
        ...
        log.info(f"[db] 베팅 기록 삽입: bet_id={cur.lastrowid} | {event_title}")
    ...
    def settle_bet(
        ...
        with self._connect() as conn:
            conn.execute(
                """
                UPDATE bets
                SET outcome = ?, pnl_usdc = ?, settled_at = ?
                WHERE id = ?
                """,
        ...
        log.info(f"[db] 베팅 정산: bet_id={bet_id} | {outcome} | P&L=${pnl_usdc:+.2f}")
    ...
    def get_bet_by_order_id(self, order_id: str) -> dict | None:
        """order_id로 베팅 기록 조회."""
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM bets WHERE order_id = ?", (order_id,)
            ).fetchone()
        return dict(row) if row else None

    def get_recent_outcomes(self, n: int = 10) -> list[str]:
        """최근 N개 정산된 베팅의 결과 반환 (최신순)."""
        with self._connect() as conn:
            rows = conn.execute(
                """
